		...
		for (var i = 0 ; i < tagTable.length ; i++) {
         ...
         var tag;
			while (tag = tags.iterateNext()) {
	         //alert(tagTable[i].tag + ' ' + (new XMLSerializer()).serializeToString(tag) );
				++foundTag;
            ...
            alert(JSON.stringify((new XMLSerializer()).serializeToString(tag)));
            now.content = tag.firstChild.textContent;    // Tu test

				last = now;               // the last result
